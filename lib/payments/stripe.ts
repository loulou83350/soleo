...
// Initialisation lazy — évite le crash au chargement quand STRIPE_SECRET_KEY
// n'est pas configuré (BILLING_ENABLED=false en dev).
export function getStripe(): Stripe {
  const key = process.env.STRIPE_SECRET_KEY;
  if (!key) {
    throw new Error('STRIPE_SECRET_KEY non configuré. Activez BILLING_ENABLED pour utiliser Stripe.');
  }
